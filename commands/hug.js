...
exports.function = async (parameters) => {
    ...
    if (!args[1]) {
        await message.reply(`${await db.get('trans', lang, 'usage')}\`${prefix}${await db.get('trans', lang, 'hug_command')}\`!`);
    }
    else {
        ...
        embed.setAuthor(`${message.author.username} ${await db.get('trans', lang, 'hug_text')} ${user}`, message.client.user.displayAvatarURL);
