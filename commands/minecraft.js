...
exports.function = async (parameters) => {
    ...
    if(!serverIP) {
        await message.reply(`${await db.get('trans', lang, 'usage')}\`${prefix}${await db.get('trans', lang, 'mc_command')}\`!`);
    } else {
        ...
        embed.setAuthor(await db.get('trans', lang, 'mc_title'), message.client.user.displayAvatarURL);

        if(server.online) {
            embed.setColor('80ba27');
            embed.addField('Status', 'Online', true);
            embed.addField(await db.get('trans', lang, 'mc_players'), `${server.players.now}/${server.players.max}`, true);
            embed.addField(await db.get('trans', lang, 'mc_version'), server.server.name, true);
            embed.addField('MOTD', '```\n' + server.motd + '\n```');
        } else {
